...
public class AddCourse extends JFrame implements ActionListener {
  ...
  static public int hasCourse(String id) { // 教师开课前检查课程是否已经存在
      ...
      if (idt.getText().equals("") || namet.getText().equals("") || greditt.getText().equals("")
        ...
        if (hasCourse(idt.getText()) == 1) {
          ...
          Course course = new Course(idt.getText(), namet.getText(), teacherIdt.getText(), teacherNamet.getText(),
              greditt.getText(), classHt.getText());
          ...
          String s = System.getProperty("user.dir") + "\\data\\grade\\" + course.getCourseName() + ".txt";
          String s1 = System.getProperty("user.dir") + "\\data\\course_student\\" + course.getCourseName()
              + "_student.txt";
          System.out.println(s);
          System.out.println(s1);
          File gradeFile = new File(s);
          File studentFile = new File(s1);
          try {
            FileWriter gradefw = new FileWriter(gradeFile);
          ...
          try {
            FileWriter sffw = new FileWriter(studentFile);
            BufferedWriter sfbw = new BufferedWriter(sffw);
            sfbw.write("");
            sfbw.close();
            sffw.close();
          } catch (IOException e2) {
